 ...
 * Member Operator new
 */
// NOTE: if we only define a new operator for dog, it will cause problems when we try to create a yellowdog.
class dog {
   ...
   public:
   static void* operator new(std::size_t size) throw(std::bad_alloc)
   {
       //first solution, handle the allocation in the base class, dog.
       //if it's not a dog, then just call the global default new operator.
      if (size == sizeof(dog))
...
class yellowdog : public dog {
   int age;
   //2nd solution, define a new operator for yellowdog too.
   static void* operator new(std::size_t size) throw(std::bad_alloc) 
};
...
// See any problem?
//
// How about a virtual static operator delete? NO, function can't be virtual and static at the same time.
//
// Solution:
//   virtual ~dog() {}
